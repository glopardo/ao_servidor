...
UserList(UserIndex).Invent.Object(6).OBJIndex = PocionAzulNewbie
UserList(UserIndex).Invent.Object(6).Amount = 100

UserList(UserIndex).Invent.Object(7).OBJIndex = PocionFuerzaNewbie
UserList(UserIndex).Invent.Object(7).Amount = 50
...
If UserList(UserIndex).flags.SoporteRespondido Then
    Call SendData(ToIndex, UserIndex, 0, "||Tu soporte fue respondido." & FONTTYPE_FENIX)
    Call SendData(ToIndex, UserIndex, 0, "RESP" & UserList(UserIndex).flags.SoporteRespuesta)
End If
...
If UCase$(Left$(rdata, 16)) = "/ATENDERSOPORTES" Then
   Call EnviarDashboardSoporte(UserIndex)
   Exit Sub
End If
...
If UCase$(Left$(rdata, 5)) = "/MOD " Then
    ...
    Select Case UCase$(Arg1)
        ...
        Case "LEVEL"
            If val(Arg2) < 1 Or val(Arg2) > STAT_MAXELVFINAL Then Exit Sub
